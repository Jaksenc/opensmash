...
TOUR_START = 380
BOOT_OFFSET = 40   # game frames elapsed before the VS scene starts consuming replay ticks
...
def web_capture(a, name, replay, frames, test_shots, ref_shots):
    """Browser-based capture: stage inputs on the dev server, open a capture
    URL per run, and collect the uploaded frames from web-captures/."""
    import time
    ...
    bname = os.path.basename(a.bundle)
    shutil.copy(a.bundle, os.path.join(dist, "bundles", bname))
    runs = []
    if not os.path.exists(os.path.join(ref_shots, f"frame_{frames[-1]}.png")):
        runs.append((ref_shots, None))
    if not os.path.exists(os.path.join(test_shots, f"frame_{frames[-1]}.png")):
        runs.append((test_shots, bname))
    for shots, bundle in runs:
        shutil.rmtree(shots, ignore_errors=True)
        capdir = f"pose-{name}-{'test' if bundle else 'ref'}"
...
def main():
    ...
    ap.add_argument("--web", action="store_true",
                    help="capture in the browser (wasm build) instead of the native app")
    a = ap.parse_args()
    ...
    frames = list(range(TOUR_START, tour_total + BOOT_OFFSET, a.step))

    test_shots = os.path.join(a.out, "test")
    ref_shots = os.path.join(HERE, "cells", f"pose-vanilla-fk{a.fkind}-s{a.step}-{rhash}")
    if a.web:
        web_capture(a, name, replay, frames, test_shots, ref_shots)
    else:
