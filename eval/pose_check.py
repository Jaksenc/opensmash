#!/usr/bin/env python3
"""Fast per-target iteration loop: convert one character onto samus, print the
...
SCRATCH = os.path.join(HERE, "pose_check")
...
def main():
    ...
    ap.add_argument("--target", default="samus",
                    help="mario/samus/luigi/link/captain (needs eval/streams/fk<K>.json)")
    ...
    FKS = {"mario": 0, "samus": 3, "luigi": 4, "link": 5, "captain": 7}
    fk = FKS[args.target]
    tag = f"{args.target}-{args.tag}"
    bundle = os.path.join(SCRATCH, f"{args.char}-{tag}.json")
    ...
    if args.target == "mario":
        ...
        cmd += ["--target", f"skels/{args.target}.profile.json",
                ...
                f"skels/{args.target}.skel", bundle]
    ...
    r = subprocess.run(["python3", "eval/stretch_eval.py", bundle,
                        f"eval/streams/fk{fk}.json", "--target", args.target,
    ...
    H = 380
    cells = []
    for f, lbl in ((459, "walk"), (605, "run"), (729, "idle"),
                   (812, "jab"), (976, "utilt"), (1116, "fsmash")):
    ...
    for i, (lbl, im) in enumerate(cells):
        dr.text((W*i+6, 6), f"{lbl}  [{args.char} {tag}]", fill=(0, 0, 0))
